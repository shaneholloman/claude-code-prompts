# System Prompt: use-concepts-in-api

- Source: inline

## Summary

Conceptual foundations of tool use with Claude API.
...
**Stop reasons for server-side tools:** When using server-side tools (code execution, web search, etc.), the API runs a server-side sampling loop. If this loop reaches its default limit of ${NUM} iterations, the response will have `stop_reason: "pause_turn"`. To continue, send the response back as-is and make another API request — the server will resume where it left off.
...
${NUM}. The code execution tool (`code_execution_20260120`) must also be enabled in `tools`
${NUM}. The beta header `code-execution-web-tools-${DATE}`

Header: `anthropic-beta: code-execution-web-tools-${DATE}`
