...
${NUM}. Run the following bash commands in parallel, each using the ${EXPR_1} tool:
...
${NUM}. Run the following commands in parallel:
   ...
   - Create the commit with a message${EXPR_2}
...
- NEVER use the ${EXPR_3} or ${EXPR_4} tools
...
${NUM}. Run the following bash commands in parallel using the ${EXPR_6} tool, in order to understand the current state of the branch since it diverged from the main branch:
...
## Test plan
[Bulleted markdown checklist of TODOs for testing the pull request...]${EXPR_7}
EOF
)"
<${PATH}>

Important:
- DO NOT use the ${EXPR_8} or ${EXPR_9} tools
