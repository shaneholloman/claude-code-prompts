...
**Don't import-and-call.** `import { foo } from '.${EXPR_2}'` then
...
git diff @{u}.. --stat                # full range, not HEAD~${EXPR_3}
gh pr diff                            # if in a PR context
```

State the commit count in your report. Large diff truncating? Redirect:
`git diff @{u}.. > ${EXPR_4}` then Read it. No diff at all → say so, stop.
...
|---|---|---|
| CLI / TUI | terminal | type the command, capture the pane — [example](examples${EXPR_5}) |
| Server / API | socket | send the request, capture the response — [example](examples${EXPR_6}) |
| GUI | pixels | drive it under xvfb${EXPR_7}, screenshot |
| Library | package boundary | sample code through the public export — `import pkg`, not `import .${EXPR_8}` |
...
**Check `.claude${EXPR_9}` first — even if you already know how to
build and run.** A matching `verifier-*` skill is the repo's
evidence-capture protocol: it wraps the session in whatever
recording${EXPR_10} mechanism the review pipeline consumes. Drive
...
- **`run-*` but no matching verifier** → use its build${EXPR_12}
  primitives as your handle.
- **Neither** → cold start from README${EXPR_13} Timebox
  ~15min. Stuck → BLOCKED with exactly where, plus a filled-in
  `${EXPR_14}` prompt. Got through → mention
  `${EXPR_15}` in your report so next time is faster.
...
→ clean `error: --from requires a value`, exit ${EXPR_16}." That the author
...
name`, bind `:${EXPR_17}`, `mktemp -d`. You share a namespace with your host.
...
**Method:** <how you got a handle — which verifier${EXPR_18}, or
cold start; what you launched>

### Steps

Each step is one thing you did to the **running app** and what it
showed. Build${EXPR_19} are setup, not steps. Test runs and
typecheck don't belong here — they're CI's output.

${EXPR_20}. ✅/❌/⚠️/🔍 <what you did to the running app> → <what you observed>
...
the feature — image path for GUI${EXPR_21}, code block for library${EXPR_22};
omit for build${EXPR_23}>
...
you relaying it isn't an observation. Claim${EXPR_24} mismatch, pre-existing
...
No partial pass. "${EXPR_26} of ${EXPR_27} passed" is FAIL until ${EXPR_28} passes or is
explained away.
