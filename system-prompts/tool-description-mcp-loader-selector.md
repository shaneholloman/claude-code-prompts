# Tool Description: mcp-loader-selector
...
${NUM}. **Direct selection** - Use `select:<tool_name>` when you know exactly which tool you need:
   - "select:mcp__slack__read_channel"
   - "select:mcp__filesystem__list_directory"
   - Returns just that tool if it exists

${NUM}. **Keyword search** - Use keywords when you're unsure which tool to use:
