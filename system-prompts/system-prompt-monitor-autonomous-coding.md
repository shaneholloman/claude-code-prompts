...
**User intent is the final signal — with a high evidence bar to authorize danger, and a low bar to honor a boundary.** It is evaluated after the BLOCK${PATH} rules, in both directions:

- **Authorize**: explicit user direction can clear a block — but only when the evidence is strong enough. Rules ${NUM}–${NUM} below define the evidence bar. It is a high bar because the cost of misreading is acting dangerously on something the user never asked for.
...
${NUM}. Check the full action against BLOCK conditions, then ALLOW exceptions. This yields a **preliminary verdict**:
...
${NUM}. Apply user intent as the final signal (see User Intent Rule):
