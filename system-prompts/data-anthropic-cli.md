# System Prompt: data-anthropic-cli

- Source: native-reference-match

## Summary

Anthropic CLI (`ant`) The `ant` CLI exposes every Claude API resource as a shell subcommand.
...
```sh
# macOS
brew install anthropics${PATH}
xattr -d com.apple.quarantine "$(brew --prefix)${PATH}"

# Linux / WSL — pick the release from github.com${PATH}
curl -fsSL "${URL}${EXPR_1}${PATH}${EXPR_2}_$(uname -s | tr A-Z a-z)_$(uname -m | sed -e s${PATH} -e s${PATH}).tar.gz" \
  | sudo tar -xz -C ${PATH} ant

# Or from source (Go ${NUM}+)
go install github.com${PATH}@latest
```
...
```sh
ant models list
ant messages create --model {{OPUS_ID}} --max-tokens ${NUM} --message '{role: user, content: "Hello"}'
...
`--transform` takes a [GJSON path](${URL}). On list endpoints it runs **per item**, not on the envelope.
...
```sh
ant beta:agents create --name "Researcher" --model '{id: {{SONNET_ID}}}' --system @.${PATH}

ant messages create --model {{OPUS_ID}} --max-tokens ${NUM} \
  --message '{role: user, content: [
    {type: document, source: {type: base64, media_type: application${PATH}, data: "@.${PATH}"}},
    {type: text, text: "Extract the text from this scanned document."}
  ]}' \
  --transform 'content.${NUM}.text' -r
```
...
This is the recommended flow for defining agents and environments — check the YAML into your repo and sync via `create` (first time) / `update` (thereafter). See `shared${PATH}` for the field reference.
...
ant beta:agents update --agent-id "$AGENT_ID" --version ${NUM} < summarizer.agent.yaml
```
...
ant beta:sessions:events list --session-id "$SID" --transform 'content.${NUM}.text' -r
...
ant beta:sessions:events send --session-id "$SID" > ${PATH} <<'YAML'
events:
  - type: user.message
    content:
      - type: text
...
while IFS= read -r -u "$stream" line; do
  case "$line" in
    ...
    type:\ session.error)
      ...
      case "$next" in err:\ *) msg=${EXPR_3} ;; *) msg=unknown ;; esac
      printf '\
[Error: %s]\
' "$msg"; break ;;
    type:\ *) type=${EXPR_4} ;;
...
This works for interactive exploration and demos. For application code that needs to react to `agent.tool_use` / `agent.custom_tool_use` events, reconnect after drops, or dedup against `events.list`, use the SDK — see `shared${PATH}`.
...
```sh
FIRST=$(ant beta:agents list --transform id -r --max-items ${NUM})
...
```sh
ant beta:agents retrieve --agent-id bogus --transform-error error.message --format-error yaml ${NUM}>&${NUM}
```

Shell completion: `ant @completion {zsh|bash|fish|powershell}`.

For the full, always-current reference (including per-endpoint flags), WebFetch the **Anthropic CLI** URL in `shared${PATH}`.
