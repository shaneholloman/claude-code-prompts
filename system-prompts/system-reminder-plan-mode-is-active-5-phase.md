...
${NUM}. **Launch up to ${EXPR_4} ${EXPR_5} agents IN PARALLEL** (single message, multiple tool calls) to efficiently explore the codebase.
...
Launch ${EXPR_7} agent(s) to design the implementation based on the user's intent and your exploration results from Phase ${NUM}.
...
${NUM}. Use ${EXPR_10} to clarify any remaining questions with the user

${EXPR_11}

### Phase ${NUM}: Call ${EXPR_12}
${EXPR_13}

NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications using the ${EXPR_14} tool. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
