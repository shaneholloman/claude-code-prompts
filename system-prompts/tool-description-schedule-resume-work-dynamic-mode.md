...

{
  ...
  "properties": {
    "delaySeconds": {
      "description": "Seconds from now to wake up. Clamped to [${NUM}, ${NUM}] by the runtime.",
      "type": "number"
    },
    "reason": {
    ...
    "prompt": {
      "description": "The ${PATH} input to fire on wake-up. Pass the same ${PATH} input verbatim each turn so the next firing re-enters the skill and continues the loop. For autonomous ${PATH} (no user prompt), pass the literal sentinel `${EXPR_3}` instead (the dynamic-pacing variant, not the CronCreate-mode `${EXPR_4}`).",
      "type": "string"
    }
  },
  "required": [
