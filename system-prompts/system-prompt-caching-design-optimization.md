# System Prompt: caching-design-optimization
...
- Minimum cacheable prefix is model-dependent (typically ${NUM}–${NUM} tokens). Shorter prefixes silently won't cache even with a marker.

**Economics:** Cache writes cost ~${NUM}× base input price; reads cost ~${NUM}×. A prefix must be used in at least two requests within TTL to break even (one writes the cache, subsequent ones read it). For bursty traffic, the ${NUM}-hour TTL keeps entries alive across gaps.
