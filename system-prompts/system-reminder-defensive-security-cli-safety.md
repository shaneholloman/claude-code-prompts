# System Reminder: defensive-security-cli-safety
...
# Tool usage policy${EXPR_10}mcp__${EXPR_11}__${EXPR_12}
...
```
Grep with pattern="<search term>" path="${EXPR_17}/" glob="**${PATH}"
```

**Session transcript logs** (full conversation history):
```
Grep with pattern="<search term>" path="${EXPR_18}/" glob="*.jsonl"
```
