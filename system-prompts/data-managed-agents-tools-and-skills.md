# System Prompt: data-managed-agents-tools-and-skills
...
|---|---|---|
| **Prebuilt Claude Agent tools** (`agent_toolset_20260401`) | Anthropic, on the session's container | File ops, bash, web search, etc. Enable all at once or configure individually with `enabled: true${PATH}`. |
...
${NUM}. Agent decides to use the tool → session emits a `agent.custom_tool_use` event with inputs
${NUM}. Session goes `idle` waiting for you
${NUM}. Your application executes the tool
${NUM}. You send back a `user.custom_tool_result` event with the output
${NUM}. Session resumes `running`
...
${NUM}. **Agent creation** declares which servers to connect to (`type`, `name`, `url` — no auth). The agent's `mcp_servers` array has no auth field.
${NUM}. **Vault** stores the OAuth credentials. Attach via `vault_ids` on session create.
...
**Vaults** store OAuth credentials (access token + refresh token) that Anthropic auto-refreshes on your behalf via standard OAuth ${NUM} `refresh_token` grant. This is the only way to authenticate MCP servers in the launch SDK.
...
- **Otherwise, register a custom tool:** the agent emits `agent.custom_tool_use`, your orchestrator (which already holds the credential) executes the call and returns `user.custom_tool_result` over the same authenticated event stream. No public endpoint is exposed; the sandbox never sees the secret. See `shared${PATH}` → Pattern ${NUM}.

**Do not put API keys in the system prompt or user messages as a workaround** — they persist in the session's event history.

> Formerly known internally as TATs (Tool${PATH} Access Tokens).

**Flow:**

${NUM}. Create a vault (`client.beta.vaults.create(...)`) — one per tenant${PATH}, or one shared, depending on your model
${NUM}. Add MCP credentials to it (`client.beta.vaults.credentials.create(...)`) — each credential is tied to one MCP server URL
${NUM}. Reference the vault on session create via `vault_ids: ["vlt_..."]`
${NUM}. Anthropic auto-refreshes tokens before they expire; the agent uses the current access token when calling MCP tools
...
**Max ${NUM} skills per agent.** Agent creation uses `managed-agents-${DATE}`; the separate Skills API (for managing custom skill definitions) uses `skills-${DATE}`.
...
| --------------------- | -------- | ----------------------------------------------- |
| Create Skill          | `POST`   | `${PATH}`                                    |
| List Skills           | `GET`    | `${PATH}`                                    |
| Get Skill             | `GET`    | `${PATH}{id}`                               |
| Delete Skill          | `DELETE` | `${PATH}{id}`                               |
| Create Version        | `POST`   | `${PATH}{id}${PATH}`                      |
| List Versions         | `GET`    | `${PATH}{id}${PATH}`                      |
| Get Version           | `GET`    | `${PATH}{id}${PATH}{version}`            |
| Delete Version        | `DELETE` | `${PATH}{id}${PATH}{version}`            |
