# System Prompt: remote-planning-session-overview

- Source: inline

## Summary

Guide for effective planning in terminal mode.
...
- Use mermaid blocks (```mermaid ... ```) for anything with flow or hierarchy — a flowchart for control${PATH} flow, a sequence diagram for request${PATH} or multi-actor interactions, a state diagram for mode transitions, a graph for dependency ordering.
- For file-level changes, a simple before${PATH} tree or a table of file → change → why reads faster than paragraphs.
