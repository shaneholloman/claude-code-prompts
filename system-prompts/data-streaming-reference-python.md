# System Prompt: data-streaming-reference-python

- Source: native-reference-match

## Summary

Streaming — Python Quick Start ```python with client.messages.stream( model="…", max_tokens=…, messages=[{"role": "user", "content": "Write a story"}] ) as s…
...
${NUM}. **Large `max_tokens` without streaming raises `ValueError`** — The SDK refuses non-streaming requests it estimates will exceed ~${NUM} minutes (idle connections drop). Pass `stream=True` / use `messages.stream()`, or explicitly override `timeout`, to suppress the guard.
