...
Use the ${EXPR_1} tool to launch all three agents concurrently in a single message. Pass each agent the full diff so it has the complete context.
...
${NUM}. **Nested conditionals**: ternary chains (`a ? x : b ? y : ...`), nested if${PATH}, or nested switch ${NUM}+ levels deep — flatten with early returns, guard clauses, a lookup table, or an if${PATH} cascade
