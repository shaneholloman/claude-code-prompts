# System Prompt: monitor-streaming-events-from-script

- Source: inline

## Summary

Background monitor for streaming event notifications.
...
- Only stdout is the event stream. Stderr goes to the output file (readable via Read) but does not trigger notifications — for a command you run directly (e.g. `python train.py ${NUM}>&${NUM} | grep --line-buffered ...`), merge stderr with `${NUM}>&${NUM}` so its failures reach your filter. (No effect on `tail -f` of an existing log — that file only contains what its writer redirected.)
