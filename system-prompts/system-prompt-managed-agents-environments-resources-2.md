# System Prompt: managed-agents-environments-resources-2
...
| Archive          | `POST`   | `${PATH}{id}${PATH}`            | Makes it **read-only**; existing sessions continue, new sessions cannot reference it. No unarchive — terminal state. |
...
- The filter parameter is **`scope_id`** (REST query param `?scope_id=<session_id>`). The SDK's files resource auto-adds only the `files-api-${DATE}` header, so pass `betas: ["managed-agents-${DATE}"]` explicitly (or both headers on raw HTTP) — without it the API may reject `scope_id` as an unknown field. Requires `@anthropic-ai${PATH}` ≥ ${NUM}.${NUM} / `anthropic` (Python) ≥ ${NUM}.${NUM} — older versions don't type `scope_id`. The `ant` CLI does **not** expose this flag yet; use the SDK or curl.
...
| List             | `GET`    | `${PATH}?scope_id=...`              | `client.beta.files.list({ scope_id, betas: ["managed-agents-${DATE}"] })` |
...
The `scope_id` filter on List scopes the results to files written to `${PATH}` by that session. Without the filter, you get all files uploaded to your account.
