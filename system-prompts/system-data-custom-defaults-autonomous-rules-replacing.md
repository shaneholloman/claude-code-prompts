...
# Raw Prompt Text
(?:${EXPR_1}${EXPR_2}npm view ${EXPR_3}@${EXPR_4} version)?${EXPR_5} loaded with errorswhat's scheduled, the cron expression, the human-readable cadence, that recurring tasks auto-expire after ${EXPR_6} days, and that they can cancel sooner with ${EXPR_7} (include the job ID). Mention this is the autonomous default and that the autonomous-loop instructions are baked in.stable## allow (custom rules replacing defaults)
Custom:
${EXPR_8}
