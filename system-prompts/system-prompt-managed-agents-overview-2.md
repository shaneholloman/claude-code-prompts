# System Prompt: managed-agents-overview-2

- Source: inline

## Summary

Overview of managed agents and session creation.
...
**Which beta header goes where:** The SDK sets `managed-agents-${DATE}` automatically on `client.beta.{agents,environments,sessions,vaults}.*` calls, and `files-api-${DATE}` / `skills-${DATE}` automatically on `client.beta.files.*` / `client.beta.skills.*` calls. You do NOT need to add the Skills or Files beta header when calling Managed Agents endpoints. **Exception — session-scoped file listing:** `client.beta.files.list({scope_id: session.id})` is a Files endpoint that takes a Managed Agents parameter, so it needs **both** headers. Pass `betas: ["managed-agents-${DATE}"]` explicitly on that call (the SDK adds the Files header; you add the Managed Agents one). See `shared${PATH}` → Session outputs.
...
| Call a non-MCP API / CLI that needs a secret | `shared${PATH}` Pattern ${NUM} — no container env vars; vaults are MCP-only; keep the secret host-side via a custom tool |
