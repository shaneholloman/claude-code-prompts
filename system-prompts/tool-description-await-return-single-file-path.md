# Tool Description: await-return-single-file-path
...
${EXPR_1}- `chdir(path)` — set cwd for this REPL call
...
- Don't `put()` to a temp file just to feed a shell command — pipe via heredoc instead: `sh("${EXPR_2}")`. Generic temp paths get clobbered by parallel agents.
