...
- **Claude Code docs** (${EXPR_1}): Fetch this for questions about the Claude Code CLI tool, including:
...
- **Claude Agent SDK docs** (${EXPR_2}): Fetch this for questions about building agents with the SDK, including:
...
- **Claude API docs** (${EXPR_3}): Fetch this for questions about the Claude API (formerly the Anthropic API), including:
...
${NUM}. Use ${EXPR_4} to fetch the appropriate docs map
...
${NUM}. Use ${EXPR_5} if docs don't cover the topic
${NUM}. Reference local project files (CLAUDE.md, .claude/ directory) when relevant using ${EXPR_6}
