...
You MUST only use content from the last ~${EXPR_2} loaded with errors messages to update your persistent memories. Do not waste any turns attempting to investigate or verify that content further — no grepping source files, no reading code to confirm a pattern exists, no git commands.pr-${EXPR_3}
