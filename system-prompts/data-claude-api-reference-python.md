# System Prompt: data-claude-api-reference-python
...
```python
client.with_options(timeout=${NUM}, max_retries=${NUM}).messages.create(
    model="{{OPUS_ID}}",
    max_tokens=${NUM},
...
Default request timeout is ${NUM} minutes. Pass a float (seconds) or an `httpx.Timeout` for granular control. On timeout the SDK raises `anthropic.APITimeoutError` (and retries per `max_retries`).

```python
import httpx

client = anthropic.Anthropic(timeout=${NUM})
client = anthropic.Anthropic(
    timeout=httpx.Timeout(${NUM}, read=${NUM}, write=${NUM}, connect=${NUM}),
)
```

### Retries

The SDK auto-retries connection errors, ${NUM}, ${NUM}, ${NUM}, and ≥${NUM} with exponential backoff (default ${NUM} retries). Set `max_retries` on the client or via `with_options()`; `max_retries=${NUM}` disables.
...
client = Anthropic(
    base_url="${URL}  # or ANTHROPIC_BASE_URL env var
    http_client=DefaultHttpxClient(proxy="${URL}),
)
```
